...
class Hero():    
    ...
    def vision(self):      
        ...
        while self.stop_while == False:  
            for check in wall_list:
                if check.check_collision(self.vision_bullet[0], self.vision_bullet[1]) == True:
                    self.vision_list.append([self.vision_bullet, check.wall_crd])
    ...
    def blit_2D(self):
        #отрисовываю героя
        pg.draw.rect(win, BLUE, (self.hero_crd[0], self.hero_crd[1], W//self.xy_matrix[0], H//self.xy_matrix[1]))
